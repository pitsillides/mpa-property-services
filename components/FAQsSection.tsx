...
const faqs: FAQItem[] = [
  {
    ...
    questionRU: "Что означает совместно принадлежащее здание?",
...
(c) Separate buildings on the same plot may also be treated as jointly owned, even if they are not divided horizontally.`,
    answerRU: `(а) Совместно принадлежащим считается здание, состоящее как минимум из 5 единиц. Единица — это часть здания, которая может использоваться самостоятельно как отдельная и полноценная собственность.

(б) Здания с двумя (2) до четырёх (4) единиц также могут быть признаны совместно принадлежащими по решению Директора, после заявления заинтересованных сторон.

(в) Отдельные строения на одном участке также могут считаться совместно принадлежащими, даже если они не разделены горизонтально.`,
  },
  {
    questionGR: "Πώς η εταιρεία σας κατανέμει τα κοινόχρηστα έξοδα;",
    questionEN: "How are common expenses allocated?",
    questionRU: "Как распределяются общие расходы?",
...
(b) For new buildings with a building permit after 1993, the share is defined by the property owner so that each buyer knows in advance the share they are purchasing in the jointly owned property.`,
    answerRU: `Владельцы оплачивают общие расходы согласно своей доле в совместной собственности, как это предусмотрено законом и соответствующими правилами.
...
If this is not successful, the Management Committee must cover the building’s needs and then proceed with legal action against the non-paying owner, in accordance with the law.`,
    answerRU: `Сначала мы стараемся связаться с владельцем и найти способ урегулировать задолженность.

Если результата нет, Управляющий комитет должен покрыть текущие потребности здания, а затем действовать юридически против владельца, который не платит, в соответствии с законом.`,
  },
  {
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="mt-2 text-3xl font-semibold text-[#12133c]">
            {pick(lang, {
              ...
              ru: "Частые вопросы о совместных зданиях и общих расходах",
            })}
          </h2>
          ...
          <p className="mt-3 text-sm text-slate-600 max-w-2xl mx-auto">
            {pick(lang, {
              ...
              ru: "Ответы на основные вопросы о законодательстве, общих расходах и работе совместных зданий.",
            })}
          </p>
        </div>
